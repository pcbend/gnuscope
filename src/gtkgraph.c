...
GType gtk_graph_get_type(void) {
  ...
  if (!graph_type) {
    ...
    const GTypeInfo graph_info = {sizeof(GtkGraphClass),
                                  NULL,
                                  NULL,
    ...
    graph_type = g_type_register_static(gtk_widget_get_type(), "GtkGraph", &graph_info, 0);
  }
  return graph_type;
}
...
void gtk_graph_set_num_lines(GtkGraph *graph, int num_lines) {
  ...
  if (graph->values == NULL) {
    ...
    graph->values = (gint **)realloc(graph->values, sizeof(gint *) * graph->num_lines);
  }
  ...
  if (graph->title == NULL) {
    ...
    graph->title = (char **)realloc(graph->title, sizeof(char *) * graph->num_lines);
  }
  for (i = 0; i < graph->num_lines; i++) {
...
void gtk_graph_add_segment(GtkGraph *graph, float x, float y) {
  ...
  if (graph->num_segments == 0) {
    ...
    if (graph->num_segments > 0) {
      ...
      graph->segments_x = (float *)realloc(graph->segments_x, graph->num_segments * sizeof(float));
      graph->segments_y = (float *)realloc(graph->segments_y, graph->num_segments * sizeof(float));
...
void gtk_graph_draw(GtkWidget *widget, GdkRectangle *area) {
  ...
  gdk_draw_rectangle(graph->pixmap, widget->style->white_gc, TRUE, 0, 0, widget->allocation.width,
  ...
  if (binsize < binsizeforce)
    binsize = binsizeforce;
  ...
  */
  ...
  case GTK_GRAPH_TYPE_SEMILOG:
    /* --- for the case of SEMILOG we cannot have values less than zero, but probably don't want to
  ...
  displayused = (long double)((long double)(widget->allocation.width - 101) /
                              (long double)graph->num_values / (long double)column_width);
  ...
  case GTK_GRAPH_TYPE_LINEAR:
    for (i = 0; (i * tempfloat2) < minmaxrange[0]; i++) {
      ...
      pango_layout_get_pixel_size(layout, &x, &y);
      gdk_draw_layout(graph->pixmap, widget->style->black_gc, 30, points[0].y + 4 - y, layout);
    }
    break;
  case GTK_GRAPH_TYPE_SEMILOG:
    for (i = 0; i < 8; i++) {
  ...
  case GTK_GRAPH_TYPE_SEMISQRT:
    for (i = 0; i < 8; i++) {
      ...
      pango_layout_set_text(layout, dummystr, strlen(dummystr));
      pango_layout_get_pixel_size(layout, &x, &y);
      gdk_draw_layout(graph->pixmap, widget->style->black_gc, 30, points[0].y - y, layout);
    }
    break;
  default:
    break;
  ...
  display_interval =
      ceil((float)abs(currentrange[1] - currentrange[0] + 1) / (float)num_to_display);
  ...
  for (i = 0; i < num_to_display; i++) {
    ...
    case GTK_GRAPH_SCALE_CALIBRATED:
      sprintf(dummystr, " %4.0f ",
              (float)(graph->calibration[0] + graph->calibration[1] * (float)(temp_channel + 1) +
                      graph->calibration[2] * pow(temp_channel + 1, 2)));
      break;
    ...
    pango_layout_set_text(layout, dummystr, strlen(dummystr));
    pango_layout_get_pixel_size(layout, &x, &y);
    gdk_draw_layout(graph->pixmap, widget->style->black_gc, points[0].x - x / 2,
                    widget->allocation.height - 7 - y, layout);
  }

  //--ddc aug11 the LAST point is handled seperately.  You'll need the text for the label FIRST
  //since the position of the hash mark depends on it!
  ...
  if (x < display_interval / 2) {
    ...
    gdk_draw_layout(graph->pixmap, widget->style->black_gc, points[1].x - (float)x / 2,
                    widget->allocation.height - 7 - y, layout);
  }
  ...
  for (z = 0; z < graph->num_lines; z++) {
    ...
    pango_layout_get_pixel_size(layout, &x, &y);
    gdk_draw_layout(graph->pixmap, widget->style->black_gc, widget->allocation.width * 7 / 10,
                    15 + 12 * z - y, layout);
    ...
    for (i = 0; i < graph->num_values; i += binsize) {
      ...
      if (points[0].x == points[1].x)
        points[1].x = points[2].x = points[0].x + 1;
      gdk_draw_lines(graph->pixmap, gc, points, 3);
    }
    points[0].x = 0;
    points[0].y = widget->allocation.height - 1;
    points[1].x = widget->allocation.width;
    points[1].y = widget->allocation.height - 1;
    gdk_draw_lines(graph->pixmap, gc, points, 2);
  ...
  if (graph->num_segments) {
    ...
    for (i = 0; i < (graph->num_segments - 2); i++) {
      points[0].x =
          (graph->segments_x[i] + 0.5 - currentrange[0]) * displayused * column_width + 100;
      points[1].x =
          (graph->segments_x[i + 1] + 0.5 - currentrange[0]) * displayused * column_width + 100;
      switch (graph->type) {
      case GTK_GRAPH_TYPE_LINEAR:
        bar_height = (float)(graph->segments_y[i] - min[0]) / (float)minmaxrange[0] * (float)height;
        next_bar_height =
            (float)(graph->segments_y[i + 1] - min[0]) / (float)minmaxrange[0] * (float)height;
        break;
      case GTK_GRAPH_TYPE_SEMILOG:
        if (graph->segments_y[i] < 1)
          graph->segments_y[i] = 1;
        if (graph->segments_y[i + 1] < 1)
          graph->segments_y[i + 1] = 1;
        bar_height = (float)log(graph->segments_y[i]) / (float)minmaxrange[0] * (float)height;
        next_bar_height =
            (float)log(graph->segments_y[i + 1]) / (float)minmaxrange[0] * (float)height;
        break;
      case GTK_GRAPH_TYPE_SEMISQRT:
        if (graph->segments_y[i] < min[0])
          graph->segments_y[i] = min[0];
        if (graph->segments_y[i + 1] < min[0])
          graph->segments_y[i + 1] = min[0];
        bar_height =
            (float)sqrt(graph->segments_y[i] - min[0]) / (float)minmaxrange[0] * (float)height;
        next_bar_height =
            (float)sqrt(graph->segments_y[i + 1] - min[0]) / (float)minmaxrange[0] * (float)height;
        break;
      default:
        break;
      }
  ...
  gdk_draw_drawable(widget->window, widget->style->fg_gc[gtk_widget_get_state(widget)],
                    graph->pixmap, 0, 0, 0, 0, widget->allocation.width, widget->allocation.height);
}
  ...
  if ((outfile = fopen(sFilename, "w")) != NULL) {
    ...
    for (z = 0; z < graph->num_lines; z++) {
      ...
      case GTK_GRAPH_TYPE_LINEAR:
        ...
        for (i = 1; i < graph->num_values; i++) {
          xdif = col_width;
          ydif =
              (float)((graph->values[z][i + 1]) - graph->values[z][i]) * bar_height_multiplier[z];
      ...
      case GTK_GRAPH_TYPE_SEMILOG:
        ...
        for (i = 1; i < graph->num_values; i++) {
          xdif = col_width;
          if (graph->values[z][i] < 1)
            temp1 = 1;
          else
            temp1 = graph->values[z][i];
          if (graph->values[z][i + 1] < 1)
            temp2 = 1;
          else
            temp2 = graph->values[z][i + 1];
      ...
      case GTK_GRAPH_TYPE_SEMISQRT:
        ...
        for (i = 1; i < graph->num_values; i++) {
          xdif = col_width;
          if (graph->values[z][i] < 0)
            temp1 = 0;
          else
            temp1 = graph->values[z][i];
          if (graph->values[z][i + 1] < 0)
            temp2 = 0;
          else
            temp2 = graph->values[z][i + 1];
    ...
    case GTK_GRAPH_TYPE_SEMISQRT:
      for (i = 0; i < 10; i++) {
        ...
        fprintf(outfile, "newpath\n %f 690 moveto 0 10 rlineto stroke\n", tempy1);
        fprintf(outfile, "%f 790 moveto\n -90 rotate\n (%d) show\n 90 rotate\n", tempy1 - 7,
                (int)(pow(max[0], 2) * sqrt((float)i / (float)10)));
      }
      break;
    ...
    for (i = 0; (i * tempfloat2) <= (graph->num_values); i++) {
      ...
      case GTK_GRAPH_SCALE_CALIBRATED:
        sprintf(dummystr, "%4.0f",
                (float)(graph->calibration[0] +
                        graph->calibration[1] * (float)(tempfloat + currentrange[0]) +
                        graph->calibration[2] * pow(tempfloat + currentrange[0], 2)));
        break;
      case GTK_GRAPH_SCALE_CHANNELS:
      default:
