...
void CommandLineHandler(int argc, char *argv[])
{
  ...
  while (i < argc) {
    if ((strstr(argv[i],"--help") != NULL) || (strstr(argv[i],"-h") != NULL)) {
      ...
      printf("LineColor Num Color -- determines the color for overlayed spectra.  Num is the layer of for the color.  Color can be any valid colorstring.");
	...
	i++;
	if (i >= argc) goto stopreadingcommand;
      }
    }
...
void CommandFileHandler(char *sFilename)
{
  ...
  if ((infile = fopen(sFilename,"r")) != NULL) {
	...
	/* --- read files (using AutoDetectFileType) [in <filename>] *
	   ...
	   --- double gauss fit [doublegauss <minchan> <maxchan> <peak1> <peak2> <width1> <width2> <background>] *
	   --- fixed full width half max gauss fit [fixedFWHMgauss <minchan> <maxchan> <FWHM> <background>] *
	...
	if (strstr(dummystr,"doublegauss") != NULL) {
	  if (sscanf(dummystr,"doublegauss %d %d %d %d %f %f %f",
		     &i, &j, &k, &l, &a, &b, &c) == 7) {
	    markers[0] = i;
	    markers[1] = j;
	    markers[2] = k;
	...
	if (strstr(dummystr,"fixedFWHMgauss") != NULL) {
	  if (sscanf(dummystr,"fixedFWHMgauss %d %d %f %f",
		     &i, &j, &a,&b) == 4) {
	    markers[0] = i;
	    markers[1] = j;
	    background[0] = b;
