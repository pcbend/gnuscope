...
void DrawColoredRectangle( GdkWindow *window,
			   gint filled,
			   gint x,
			   gint y,
			   gint width,
...
void DrawColoredText( GdkDrawable *drawable,
		      GtkWidget *widget,
		      const char *colorparse,
		      gint x,
		      gint y,
...
void DrawRGBColoredRectangle(GdkDrawable *drawable,
			 GdkGC *gcd,
			 gint x, gint y,
			 gint width,
			 gint height,
...
//--ddc aug11
  ...
  if (!twodplot_type) {
    ...
    */
    ...
    twodplot_type = g_type_register_static(gtk_widget_get_type(),"GtkTwodplot",
				 &twodplot_info,0);
  }
...
void gtk_twodplot_draw (GtkWidget *widget, GdkRectangle*area)
{
  ...
  if (gtk_widget_is_drawable(widget)) {
    ...
    if (twodplot->pixmap != NULL) g_object_unref(twodplot->pixmap);

    twodplot->pixmap = gdk_pixmap_new(widget->window,
    ...
    gdk_draw_rectangle(twodplot->pixmap,
	...
	if (twodplot->density_mode == GTK_TWODPLOT_DENSITY_TYPE_COLOR) {
		...
		break;
	      case GTK_TWODPLOT_CONTOUR_TYPE_LOG:
                ...
                mag = 3 * (log((float)i + 1.01) / 
                	   log((float)(twodplot->color_depth - 1) + 1.1));
		...
		case GTK_TWODPLOT_CONTOUR_TYPE_ROOT:
		  ...
		  break;
		case GTK_TWODPLOT_CONTOUR_TYPE_INVERSE_LOG:
                  //--ddc  intensity = 255 * (1 - log((float) i + 1.01) / log((float)twodplot->color_depth+ 1.01));
                  intensity = 255 * (1 - log((float) i + 1.01) / log((float)twodplot->color_depth-1+ 1.01));
		  break;
		case GTK_TWODPLOT_CONTOUR_TYPE_INVERSE_ROOT:
	...
	if (twodplot->density_mode == GTK_TWODPLOT_DENSITY_TYPE_BW) {
	  if (i == 0) 
	    ...
	    redval = greenval = blueval = 256 * sqrt(log (i + .01) / log (twodplot->color_depth + .01));
	}
    ...
    xdisplayused = (long double) ((long double)( widget->allocation.width - 31) /
				  (long double) twodplot->x_num /
				  (long double) x_column_width );
    ydisplayused = (long double) ((long double)( widget->allocation.height - 31) /
				  (long double) twodplot->y_num /
				  (long double) y_column_width );
	...
	for (i = 0; i < width; i++) {
	  ...
	  /* --- first if the interpolation_mode is odd --- */
            if (twodplot->interpolation_mode % 2) {
              ...
              tempfloaty = (float) twodplot->y_num - 
	    ...
	    /* --- we do this by rounding down both and then adding --- */
              ...
              if (tempfloatx >= (twodplot->x_num - numpoints)) {
                ...
                if (tempfloatx < numpoints) tempx = numpoints;
                else tempx = tempfloatx;
              }
              if (tempfloaty >= (twodplot->y_num - numpoints)) {
                tempy = twodplot->y_num - numpoints - 1;
              } else {
                if (tempfloaty < numpoints) tempy = numpoints;
                else tempy = tempfloaty;
              }
	  ...
	  /* --- let's do the actual interpolation --- */
            ...
            if(tempheight[i][j]<0) tempheight[i][j]=0;  //--ddc NO NEGATIVES!
   ...
   /* --- display contours --- */
	...
	for (i = 0; i < width; i++) {
	  ...
	  if ((twodplot->plot_mode == GTK_TWODPLOT_CONTOUR_ONLY) || 
		...
		if (((tempheight[i][j] <= contour_vals[k]) && 
          ...
          if (twodplot->gate
              [(int)((float)i/(float)width*(float)twodplot->x_num)]
              [(int)((float) twodplot->y_num - 
	  ...
	  *(tempbuffer + i * 3 + j * width * 3 + 2) = (guchar) (int) blueval;
	  ...
	  gdk_draw_rgb_image(twodplot->pixmap,
    ...
    DrawColoredText(twodplot->pixmap,
		    widget,
		    "white",
		    (widget->allocation.width) * 7 / 10, 15, twodplot->title);
	...
	for (i = 10; i >= 1; i--) {
	  ...
	  sprintf(colorstr,"#%.2x%.2x%.2x",
	 		  (unsigned short)redval,(unsigned short)greenval,(unsigned short)blueval);
	...
	there are multiple problems here... this is a bug in the original code uncorrected till
      ...
      */
      ...
      //--ddc jul11 (oops, wrong dimension!) y_num_to_display = (float) width / (float) (y_string_size * 2);
      y_num_to_display = (float) height / (float) (y_string_size * 2);
      /*
      x_display_interval = (float) abs(twodplot->x_range[1] - twodplot->x_range[0]) / x_num_to_display;
      y_display_interval = (float) abs(twodplot->y_range[1] - twodplot->y_range[0]) / y_num_to_display;
      */
      ...
      x_display_interval = ceil((float)abs(twodplot->x_range[1] - twodplot->x_range[0]) / x_num_to_display);
      y_display_interval = ceil((float)abs(twodplot->y_range[1] - twodplot->y_range[0]) / y_num_to_display);
      x_num_to_display= ceil((float)abs(twodplot->x_range[1] - twodplot->x_range[0])/x_display_interval);
      y_num_to_display= ceil((float)abs(twodplot->y_range[1] - twodplot->y_range[0])/y_display_interval);
	...
	sprintf(dummystr,"%d",(int)(twodplot->xcalibs[0] + temp_channel * twodplot->xcalibs[1] + 
		temp_channel * temp_channel * twodplot->xcalibs[2]));

	pango_layout_set_text(layout,dummystr,strlen(dummystr));
	pango_layout_get_pixel_size(layout,&x,&y);
	gdk_draw_layout(twodplot->pixmap, widget->style->white_gc,  
		  points[0].x-x/2, widget->allocation.height-5-y,layout);
	...
	sprintf(dummystr,"%d",(int)(twodplot->ycalibs[0] + temp_channel * twodplot->ycalibs[1] + 
		temp_channel * temp_channel * twodplot->ycalibs[2]));

	pango_layout_set_text(layout,dummystr,strlen(dummystr));
	pango_layout_get_pixel_size(layout,&x,&y);
	gdk_draw_layout(twodplot->pixmap, widget->style->white_gc,  
			  1, widget->allocation.height-30-result-y/2,  layout);
    ...
    for (i = 0; i < twodplot->num_markers; i++) {
      gdk_draw_rectangle(twodplot->pixmap,gc,
			 0,
			 28 + (twodplot->markers_x[i] + 0.5) * x_column_width,
			 height - (twodplot->markers_y[i] + 0.5) * y_column_width - 2,
			 5,5);

      DrawColoredText(twodplot->pixmap, widget, NULL,
		      34 + (twodplot->markers_x[i] + 0.5) * x_column_width,
		      height - (twodplot->markers_y[i] + 0.5) * y_column_width - 3,
		      twodplot->marker_text[i]);
 ...
 * It can be modified.
 */
static void gtk_twodplot_size_request (GtkWidget *widget,
				    GtkRequisition *req)
{
...
static void gtk_twodplot_destroy (GtkObject *object)
{
  ...
  if (deltwodplot->pixmap != NULL) g_object_unref(deltwodplot->pixmap);
	...
	for (l = 99; l >= 0; l--) {
	  for (k = 0; k < 100; k++) {
	    ...
	    if (twodplot->interpolation_mode % 2) {
	      ...
	      tempfloaty = (float) (k + 100 * i) / (float) 500 *
		(float) twodplot->y_num;
	      numpoints = twodplot->interpolation_mode / 2;
	      /* --- make sure that everything will end up in bounds --- */
	      if (tempfloatx >= (twodplot->x_num - numpoints)) {
		tempx = twodplot->x_num - numpoints - 1;
...
/* --- normalize and set the intensity --- */
		  ...
		  case GTK_TWODPLOT_FADE_ON:
		    ...
		    case GTK_TWODPLOT_CONTOUR_TYPE_LOG:
		      intensity = 
			255 * log(tempheight[k][l] + 1.01) / log(max + 1.01) ;
		      break;
		    case GTK_TWODPLOT_CONTOUR_TYPE_ROOT:
	  ...
	  for (k = 0; k < 100; k++) {
	    for (l = 0; l < 100; l++) {
	      fprintf(outfile,"%02x %02x %02x ",
		      (guchar)(int)redval[k][l],
		      (guchar)(int)greenval[k][l],
		      (guchar)(int)blueval[k][l]);
	    }
	  }
	  fprintf(outfile," >}\n false 3 colorimage\n grestore\n");
	}
	...
	if ((twodplot->plot_mode == GTK_TWODPLOT_CONTOUR_ONLY) ||
	  ...
	  case GTK_TWODPLOT_CONTOUR_TYPE_LINEAR:
	    ...
	    fprintf(outfile,"300 730 moveto -90 rotate (Linearly Spaced Contours) show 90 rotate\n",
		    (int) max);
	  ...
	  case GTK_TWODPLOT_CONTOUR_TYPE_INVERSE_LOG:
	    for (k = 0; k < twodplot->num_contours; k++)
	      contour_vals[k] = (float) max * (1 - (log(k+1.1) / log(twodplot->num_contours + 1.1)));
	    fprintf(outfile,"300 730 moveto -90 rotate (Inverse Logrythmic Contours) show 90 rotate\n",
		    (int) max);
	    fprintf(outfile,"280 730 moveto -90 rotate (Max = %d) show 90 rotate\n",
		    (int) max);
	    fprintf(outfile,"260 730 moveto -90 rotate (%d Contours) show 90 rotate\n",
	  ...
	  case GTK_TWODPLOT_CONTOUR_TYPE_INVERSE_ROOT:
	    for (k = 0; k < twodplot->num_contours; k++)
	      contour_vals[k] = (float) max * (1 - (sqrt(k+1) / sqrt(twodplot->num_contours))) + 0.1;
	  ...
	  for (m = 0; m < twodplot->num_contours; m++) {
		...
		if ((tempheight[k][l] <= contour_vals[m]) && 
		  ...
		  fprintf(outfile,"newpath\n %f %f moveto\n %f %f lineto\n stroke\n",
			  k + 100 * i + 30.5, 500 - (l - 0.5 + 100 * j),
			  k + 100 * i + 30.5, 500 - (l + 0.5 + 100 * j));
		}  
		if  ((tempheight[k][l] <= contour_vals[m]) && 
		     (tempheight[k][l+1] >= contour_vals[m])) {
		  fprintf(outfile,"newpath\n %f %f moveto\n %f %f lineto\n stroke\n",
			  k + 100 * i + 29.5, 500 - (l + 0.5 + 100 * j),
			  k + 100 * i + 30.5, 500 - (l + 0.5 + 100 * j));
		...
		if  ((tempheight[k][l] <= contour_vals[m]) && 
		  ...
		  fprintf(outfile,"newpath\n %f %f moveto\n %f %f lineto\n stroke\n",
			  k + 100 * i + 30, 500 - (l + 0.5 + 100 * j),
			  k + 100 * i + 30.5, 500 - (l + 100 * j));
	...
	if (twodplot->density_mode == GTK_TWODPLOT_DENSITY_TYPE_BW) {
	  ...
	  if (y >= 256) y = 0;
	  if (z >= 256) z = 0;
	  fprintf(outfile,"gsave %d 700 translate 15 15 scale 1 1 8 [1 0 0 1 0 0] {<%02x %02x %02x>} false 3 colorimage grestore\n",530 - 20 * i,x,y,z);
	  fprintf(outfile,"%d 690 moveto -90 rotate (%d) show 90 rotate\n",530 - 20 * i,(int) (max * (float) i / (float) 10));
      }
    ...
    for (i = 0; i < twodplot->num_markers; i++) {
      ...
      fprintf(outfile,"%f %f moveto -90 rotate (%s) show 90 rotate\n",
	      ...
	      x_col_width * (twodplot->markers_x[i] + 0.5) + 495,
	      twodplot->marker_text[i]);
    }
    ...
    for (i = 0; (i * tempfloat2) <= twodplot->y_num; i++) {
      ...
      fprintf(outfile,"%f 550 moveto\n -90 rotate\n (%d) show\n 90 rotate\n",
	      tempy1 - 7, (int) tempfloat + ycurrentrange[0]);
    }
...
{ 
  ...
  if (twodplot->num_markers <= 0)  {
    ...
    twodplot->markers_x = (float *)realloc(twodplot->markers_x,
					  sizeof(float) * twodplot->num_markers);
    twodplot->markers_y = (float *)realloc(twodplot->markers_y,
					  sizeof(float) * twodplot->num_markers);
    twodplot->marker_text = (char **) realloc(twodplot->marker_text,
					      sizeof(char *) * 
					      twodplot->num_markers);
    twodplot->marker_text[twodplot->num_markers - 1] = (char *)
  ...
  if (strlen(text) < 8)
    ...
    i = 8;
  strncpy(twodplot->marker_text[twodplot->num_markers - 1],
	  text,i);
}
