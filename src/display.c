...
void ManualMarkerEntry(GtkWidget *widget,
		     GtkWidget *entry);
void ManualMarkerPrompt(GtkWidget *text);
void ExpandPlotEntry(GtkWidget *widget,
		     GtkWidget *entry);
void ExpandPlotPrompt(GtkWidget *text);
void SetYScaleEntry(GtkWidget *widget,
		     GtkWidget *entry);
void SetYScalePrompt(GtkWidget *text);
void MultipleDisplayEntry(GtkWidget *widget,
		     GtkWidget *entry);
void SetYScalePrompt(GtkWidget *text);
void GetCalibrationEntry(GtkWidget *widget,
		     GtkWidget *entry);
...
void DrawColoredLine( GdkDrawable *drawable,
...
gint UpdateCursorX(GtkWidget *widget, GdkEventMotion *event)
{
  ...
  if (histsize[spectra] > 0) {
    if (event->is_hint)
      ...
      */
      ...
      gdk_display_get_device_state(event->window,event->device,NULL,
				   &x,&y,&state);
      */
      gdk_display_get_pointer(event->window,NULL,
				   &x,&y,&state);
    else {
      x = event->x;
      y = event->y;
    ...
    if ((binsize > 1) && (displayused != 0)) {
      result = (float) (((float) x - 100) / (float) displayused) / (float) column_width + (float) currentrange[0] ;
    } else {
      result = (float) ((float) x - 100) / currentscaling + (float) currentrange[0];
    }
    if (result > histsize[spectra]) result = histsize[spectra] - 1;
    ...
    if (UseCalibration((int)result) == -1) {
      ...
      sprintf(dummystr,"Channel: %d     Energy: %.1f     Counts: %d",
	      (int)result+1, UseCalibration((int)result), tempsum);
    }
...
void ExpandPlot()
{
  ...
  if (abs(i) > 1) {
    ...
    if (currentrange[0] < 0) currentrange[0] = 0;
    if (currentrange[1] > histsize[spectra]) currentrange[1] = histsize[spectra];
  } else {
...
{
  int i,j;
  /* --- unfortunately we need to know which graph was clicked on last (and therefore which spectra to update --- */
  j = 0;
  for (i = 0; i < numspectra; i++) {
		     ...
		     GtkWidget *entry)
{
  ...
  numspectra = sscanf(gtk_entry_get_text(GTK_ENTRY(entry)),
		      ...
		      &spectradisplayed[0][0], &spectradisplayed[1][0], &spectradisplayed[2][0],
		      &spectradisplayed[3][0], &spectradisplayed[4][0], &spectradisplayed[5][0],
		      &spectradisplayed[6][0], &spectradisplayed[7][0], &spectradisplayed[8][0],
		      &spectradisplayed[9][0], &spectradisplayed[10][0], &spectradisplayed[11][0],
		      &spectradisplayed[12][0], &spectradisplayed[13][0], &spectradisplayed[14][0],
		      &spectradisplayed[15][0]);
  test = numspectra;
  if (numspectra <= 0) numspectra = 1;
  for (i = 0; i < numspectra; i++) {
    spectradisplayed[i][0]--;
    if (spectradisplayed[i][0] < 0) spectradisplayed[i][0] = 0;
  }

  //  printf("Entries detected %d.\n",numspectra);
  for (i = 0; i < numspectra; i++){
    if (histsize[spectradisplayed[i][0]] < 1) test = 0;
  }
  if (test) {
    spectra = spectradisplayed[0][0];
    currentrange[0] = 0;
...
void MultiInOneEntry(GtkWidget *widget, GtkWidget *entry)
{
  ...
  test = sscanf(gtk_entry_get_text(GTK_ENTRY(entry)),
		"%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
		&spectradisplayed[which][0], &spectradisplayed[which][1], 
		&spectradisplayed[which][2], &spectradisplayed[which][3], 
		&spectradisplayed[which][4], &spectradisplayed[which][5],
	        &spectradisplayed[which][6], &spectradisplayed[which][7],
		&spectradisplayed[which][8], &spectradisplayed[which][9]);
  for (i = 0; i < test; i++) {
    spectradisplayed[which][i]--;
    if (spectradisplayed[which][i] < 0) spectradisplayed[which][i] = 0;
  }
  if (test) {
...
void DisplayCurrentRange()
{
  ...
  if (numspectra > 16) numspectra = 16;
  ...
  */
  ...
  for (i = 0; i < numspectra; i++) {
    ...
    if (gtk_graph_has_segments(GTK_GRAPH(graphsdisplayed[i]))) 
      gtk_graph_clear_segments(GTK_GRAPH(graphsdisplayed[i]));
    for (y = 0; y < z; y++) {
      sprintf(dummystr,"\nSpectra: %d, Title: %s, Channels %d to %d.",
	      spectradisplayed[i][y]+1,field2[spectradisplayed[i][y]], currentrange[0]+1,currentrange[1]+1);
	...
	gtk_graph_set_calibration(GTK_GRAPH(graphsdisplayed[i]),
				  calibration[spectradisplayed[i][0]][0],
				  calibration[spectradisplayed[i][0]][1],
				  calibration[spectradisplayed[i][0]][2]);
      } else {
	gtk_graph_set_calibration(GTK_GRAPH(graphsdisplayed[i]),
				  globalcalibration[0],globalcalibration[1],globalcalibration[2]);
      }
...
void SetMarker (GtkWidget *widget, GdkEventButton *event)
{
  ...
  if (currentrange[0] == 0) i = 1;
  else i = currentrange[0];
...
void GetCalibrationEntry (GtkWidget *widget, GtkWidget *entry)
{
  ...
  test = sscanf(gtk_entry_get_text(GTK_ENTRY(entry)),"%f %f %f",
		&input[0],
		&input[1],
		&input[2]);
  if (test == 3) {
...
float UseCalibration(float chan)
{
  ...
  if (globalcalibrationset == 1) {
	...
	(calibration[spectra][2] !=0)) {
      energy = calibration[spectra][0] + 
	calibration[spectra][1] * (chan + 1) +
	calibration[spectra][2] * (chan + 1) * (chan + 1);
	//--ddc 3jun08 dbg calibration[spectra][2] * calibration[spectra][2] * (chan + 1);

    } else {
      energy = globalcalibration[0] +
	globalcalibration[1] * (chan + 1) +
...
float UseCalibrationFWHM(float chan)
{
  ...
  if (globalcalibrationset == 1) {
	...
	(calibration[spectra][1] != 0) ||
	(calibration[spectra][2] !=0)) {
      energy = 	calibration[spectra][1] * (chan + 1) +
	calibration[spectra][2] * (chan + 1) * (chan + 1);
	//--ddc 3jun08 dbg calibration[spectra][2] * calibration[spectra][2] * (chan + 1);
    } else {
      energy =	globalcalibration[1] * (chan + 1) +
	globalcalibration[2] * (chan + 1)  * (chan + 1);
	//--ddc 3jun08 dbg globalcalibration[2] * globalcalibration[2] * (chan + 1);
    }
    return(energy);
...
float UseEfficiency (float counts,float energy)
{
  ...
  if (energy > 100) {
    if ((efficiencycal[0] != 0) || (efficiencycal[1] !=0) || 
	(efficiencycal[2] != 0) || (efficiencycal[3] != 0)) {
      a = (log(energy) / 2.302585);

      b = (efficiencycal[0] - (efficiencycal[1] * a) +
	   (efficiencycal[2] * a * a) - efficiencycal[3] / energy / energy);
      c = exp(b * 2.302585);
