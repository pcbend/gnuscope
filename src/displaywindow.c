...
void TwodCreateDisplaySelectionWindow() {
  ...
  { // this is to replace deprecated gtk_list...
    ...
    locallistcol =
        gtk_tree_view_column_new_with_attributes("Spec #", locallistrenderer, "text", 0, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(locallistbox), locallistcol);

    locallistcol =
        gtk_tree_view_column_new_with_attributes("Name", locallistrenderer, "text", 1, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(locallistbox), locallistcol);

    //--ddc jan11 deprecation
    //gtk_list_set_selection_mode(GTK_LIST(locallistbox),GTK_SELECTION_MULTIPLE);
    ...
    gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(localscrollwindow), locallistbox);
  ...
  g_signal_connect(GTK_OBJECT(localentry), "activate", G_CALLBACK(PgamDisplaySelectionEntry),
                   localentry);
  g_signal_connect_swapped(GTK_OBJECT(localentry), "activate", G_CALLBACK(CloseWidget),
                           (GtkObject *)localwindow);
  ...
  g_signal_connect(GTK_OBJECT(localbutton), "clicked", G_CALLBACK(TwodDisplayWindowCallback), NULL);
...
void CreateDisplaySelectionWindow(int mode) {
  ...
  switch (mode) {
  case 1:
    g_signal_connect(GTK_OBJECT(localentry), "activate", G_CALLBACK(MultipleDisplaySameRangeEntry),
                     localentry);
    break;
  case 2:
    g_signal_connect(GTK_OBJECT(localentry), "activate", G_CALLBACK(MultiInOneEntry), localentry);
    break;
  default:
    /*///hmm.
    ...
    */
    g_signal_connect(GTK_OBJECT(localentry), "activate", G_CALLBACK(MultipleDisplayEntry),
                     GTK_OBJECT(localentry));
  }
  ...
  localbutton = gtk_button_new_with_label("Display");
  gtk_container_add(GTK_CONTAINER(localhbox), localbutton);
  g_signal_connect(GTK_OBJECT(localbutton), "clicked", G_CALLBACK(DisplayWindowCallback),
                   (int *)mode);
  g_signal_connect_swapped(GTK_OBJECT(localbutton), "clicked", G_CALLBACK(CloseWidget),
                           GTK_OBJECT(localwindow));
...
static void ListBoxSelectedCallbackDisplay(GtkTreeSelection *widget, gpointer *data) {
  ...
  if (pathlist != NULL) {
    ...
    while (pathlist) {
      ...
      if (localdata > 0 && localdata < 255)
        display_toggle[(int)localdata - 1] = 1;
...
static void TwodListBoxSelectedCallbackDisplay(GtkTreeSelection *widget, gpointer *data) {
  ...
  if (pathlist != NULL) {
    for (i = 0; i < 254; i++)
      twod_display_toggle[i] = 0;
    while (pathlist) {
      gtk_tree_model_get_iter(model, &iter, pathlist->data);
      gtk_tree_model_get(model, &iter, 0, &localdata, -1);
      if (localdata > 0 && localdata < 255)
        twod_display_toggle[(int)localdata - 1] = 1;
      pathlist = pathlist->next;
    }
    g_list_foreach(pathlist, (GFunc)gtk_tree_path_free, NULL);
    g_list_free(pathlist);
...
void TwodDisplayWindowCallback(GtkWidget *widget, gpointer *data) {
  ...
  if (j > 0) {
    ...
    ymax = 0;
    for (i = 0; i < j; i++) {
      if (xmax < big_data_info[templist[i]]->axes[0])
        xmax = big_data_info[templist[i]]->axes[0];
      if (ymax < big_data_info[templist[i]]->axes[1])
        ymax = big_data_info[templist[i]]->axes[1];
    }
    xcurrentrange[0] = 0;
    ycurrentrange[0] = 0;
