...
void UpdateSortProgress(long pos, long len) {
  ...
  if (len > 0) {
    ...
    if (sdata->nLastFrac != pvalue) {
      ...
      /* --- Update the displayed value --- */
      if ((pvalue <= 1.0) && (pvalue >= 0))
        gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(sdata->progressbar), pvalue);
...
void StartProgress() {
  ...
  g_signal_connect(GTK_OBJECT(barwindow), "delete_event", G_CALLBACK(CanWindowClose), pdata);
  //--ddc aug11
...
void StartSortProgress() {
  ...
  /* --- Hook up the destroy  --- */
  //--ddc aug11    gtk_signal_connect (GTK_OBJECT (barwindow), "delete_event",
  g_signal_connect(GTK_OBJECT(barwindow), "delete_event", G_CALLBACK(CanWindowClose), sdata);
  ...
  gtk_table_attach_defaults(GTK_TABLE(table), sdata->progressbar, 0, 2, 1, 2);
  ...
  g_signal_connect(GTK_OBJECT(localbutton), "clicked", G_CALLBACK(SortAbortCallback), NULL);
