	       ...
	       double *a)
{
  ...
  for (l = 0; l < nterms; l++) {
	...
	n = j+k;
	MatSetVal(j,k,sumx[n],array);
	if ((mode == 2) && (j == 3)) MatSetVal(j,k,sume[k],array);
	if ((mode == 2) && (k == 3)) MatSetVal(j,k,sume[j],array);
      }
...
double MatDet (struct my_matrix_struct *matrix)
{
  ...
  case 2:
    ...
    break;
  case 3:
    temp = array[0][0] * array[1][1] * array[2][2] +
  ...
  default:
    ...
    for ( i = 0; i < size; i++) {
      ...
      for ( j = 1; j < size; j++) {
	for ( k = 0; k < size; k++) {
	  if (k == i) k++; // skip the appropriate column
