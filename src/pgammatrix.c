...
void AddAutoMatrix(char *sFilename)
{
  ...
  if ((twoddata.data == NULL) && (pgammatrixdata.data == NULL))
	...
	if (databuffersize[0] == 
	  ...
	  matrixtype = 4;
	} else if (databuffersize[0] == (sizeof(short int) * tempbuffer[0] * (tempbuffer[0] + 1) / 2)) {
...
void WriteAutoMatrix(char *sFilename)
{
  ...
  default:
    /* --- a float matrix --- */
    if ((pgammatrixdata.type == 0) && 
	(strstr(dummystr,".sqr") == NULL))
      strcat(dummystr,".sqr");
    if ((pgammatrixdata.type == 3) &&
	(strstr(dummystr,".twd") == NULL))
      strcat(dummystr,".twd");
...
void ReadAutoMatrix (char *sFilename)
{
  ...
  if ((infile = fopen(sFilename,"r")) != NULL) {
	...
	twoddata.data = (short int *)
	  malloc(twoddata.databuffer[0]);
	...
	if (pgammatrixdata.data == NULL) 
	  pgammatrixdata.data = (float *) 
	    malloc(pgammatrixdata.databuffer[0]);
	else
	  pgammatrixdata.data = (float *)
	    realloc(pgammatrixdata.data,
		    pgammatrixdata.databuffer[0]);
	fread(pgammatrixdata.data,1,
	      pgammatrixdata.databuffer[0],infile);
	fread(&pgammatrixdata.databuffer[1],
	      sizeof(int),1,infile);
			 ...
			 int histdest,int add,int xory)
{
  ...
  for ( i = addlower; i <= addupper; i++) {
	...
	UpdateProgress(counter,maxcounter);
      ********/
      if (xory == 0) temphist[j] = temphist[j] + 
		       *(pgammatrixdata.data + pgammatrixdata.size * i + j);
      if (xory == 1) temphist[j] = temphist[j] +
		       *(pgammatrixdata.data + pgammatrixdata.size * j + i);
    }
    UpdateProgress(counter,maxcounter);
  }
  EndProgress();
  ...
  if (subtractlower > 0) {
	...
	UpdateProgress(counter,maxcounter);
	*/
	if (xory == 0) subhist[j] = subhist[j] +
			(float) *(pgammatrixdata.data + pgammatrixdata.size * i + j);
	if (xory == 1) subhist[j] = subhist[j] +
			 (float) *(pgammatrixdata.data + pgammatrixdata.size * j + i);
      }
      UpdateProgress(counter,maxcounter);
    }
    EndProgress();
			 ...
			 int histdest, int add)
{
  ...
  /* --- initializations --- */
  counts = 0;
  for (i = 0; i < 16384; i++) temphist[i] = 0;
  for (i = 0; i < 16384; i++) subhist[i] = 0;
  ...
  for (i = 0; i < pgammatrixdata.size; i++) {
    ...
    for ( j = addlower; j <= addupper; j++) {
      ...
      if (ix == iy) temporary = temporary + temp; // intentionally double count on x = y
	...
	if (ix == iy) subhist[i] += *(pgammatrixdata.data + ind);
	//	temp = *(pgammatrixdata.data + ind);
	//if (ix == iy) temporary = temporary - temp * asratio; // intentionally double count on x = y
    ...
    if (subtractlower > 0) temphist[i] -= subhist[i] * asratio;
  ...
  for ( i = 0; i < pgammatrixdata.size; i++)  *(histloc[histdest] + i) = rint(temphist[i]);
