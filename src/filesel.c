...
void GetFilename(char *sTitle, int WriteDialog, void (*callback)(char *)) {
  ...
  if (WriteDialog == 1) {
    filew =
        gtk_file_chooser_dialog_new(sTitle, NULL, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL,
                                    GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
  } else {
    filew =
        gtk_file_chooser_dialog_new(sTitle, NULL, GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL,
                                    GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
  }
  ...
  if (sFilename) {
    ...
    gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(filew), sFilename);
  ...
  g_signal_connect(GTK_OBJECT(GTK_FILE_CHOOSER(filew)), "response", (GCallback)FileOk, data);
