...
static void MenuYCalibration(GtkWidget *widget,
			      guint *callback_action)
{
  GetDialog(39);
}

static void MenuXCalibration(GtkWidget *widget,
			      guint *callback_action)
{
  GetDialog(38);
}

static void MenuPgamSetFadeMode(gpointer (callback_data),
				guint *callback_action)
{
...
static void MenuPgamSetColorMode(GtkWidget *widget,
			      guint *callback_action)
{
  pgamcolormode = *callback_action;
  TwodDisplayCurrentRange();
}
static void MenuPgamSetContourType(GtkWidget *widget,
			      guint *callback_action)
{
  pgamcontourtype = *callback_action;
  TwodDisplayCurrentRange();
}
static void MenuPgamSetType(GtkWidget *widget,
			      guint *callback_action)
{
...
static void MenuPgamSetInterpolation(GtkWidget *widget,
			      guint *callback_action)
{
...
static void MenuTwodReadFile(GtkWidget *widget,
			      guint *callback_action)
{
  GetFilename("Read Files",0, AutoDetectFileType);
}

static void MenuPgamSubtract2D(GtkWidget *widget,
				 guint *callback_action)
{
  GetDialog(37);
}
static void MenuPgamAdd2D(GtkWidget *widget,
				 guint *callback_action)
{
  GetDialog(36);
}
static void MenuPgamClear2D(GtkWidget *widget,
				 guint *callback_action)
{
  GetDialog(35);
}
static void MenuPgamSelect2D(GtkWidget *widget,
				 guint *callback_action)
{
...
static void MenuPgamPrint(GtkWidget *widget,
			  guint *callback_action)
{
...
static void MenuPgamSetTwodTitle(GtkWidget *widget,
			  guint *callback_action)
{
  GetDialog(33);
}

static void MenuPgamWritePostscript(GtkWidget *widget,
			  guint *callback_action)
{
  GetFilename("Postscript Output",1, WritePgamPostscript);
}

static void MenuPgamSetNumContours(GtkWidget *widget,
				    guint *callback_action)
{
  GetDialog(31);
}

static void MenuPgamBWDensity(GtkWidget *widget,
			      guint *callback_action)
{
...
static void MenuPgamColorDensity(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamInverseRootContours(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamInverseLogContours(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamRootContours(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamLogContours(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamLinearContours(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamDisplayContour(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamDisplayDensity(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamDisplayContourAndDensity(GtkWidget *widget,
				    guint *callback_action)
{
...
static void MenuPgamWritePgamTwod (GtkWidget *widget, 
				     guint *callback_action)
{
  GetFilename("Write Pgam Matrix",1, WritePgamTwod);
}

static void MenuPgamReadPgamTwod (GtkWidget *widget, 
				     guint *callback_action)
{
 ...
 *
 * Assigns the integer possition of the "on" toggled bits for a particular channel to a pointer passed to it
...
void PgamSubtract2DEntry(GtkWidget *widget, GtkWidget *entry)
{
  ...
  if (sscanf(gtk_entry_get_text(GTK_ENTRY(entry)),"%d %d %d",&a,&b,&c) == 3) {
	...
	big_data_info = (struct bigdata **) realloc(big_data_info,
					      sizeof(struct bigdata *) * pgammax);
	big_data_info[c] = 
	  BigDataStructNew(big_data_info[a]->axes[0],big_data_info[a]->axes[1]);
...
void PgamClear2DEntry(GtkWidget *widget, GtkWidget *entry)
{
  ...
  if (sscanf(gtk_entry_get_text(GTK_ENTRY(entry)),"%d",&i) == 1)  {
	...
	pgammax = pgammax - 1;
	if (pgam >= pgammax) pgam = pgammax - 1;
      } else {
 ...
 */
void KinmatInfoStructAdd(struct kinmat_info_struct *stru,
	  float ex, float en)
{
  int temp;
...
void TwodGateFill()
{
  ...
  if ((gate_info != NULL) && (big_data_info != NULL)) {
    ...
    TwodDisplayCurrentRange();
  } else GetMessageDialog("There is either no gate info or 2D info in memory.");
}
...
static gint TwodGateSet(GtkWidget *widget,GdkEventKey *event)
{
  ...
  if (big_data_info != NULL) {
	...
	if (abs(twodmarkers[0].x - firstpoint.x) != 0) {
	  slope = (float) (twodmarkers[0].y - firstpoint.y) /
	    (float) (twodmarkers[0].x - firstpoint.x);
	} else {
	  slope = 1000000;
	}
	y = Min(twodmarkers[0].x,firstpoint.x);
	z = Max(twodmarkers[0].x,firstpoint.x);
	if (twodmarkers[0].x == y) x = twodmarkers[0].y;
	else x = firstpoint.y;
	for ( i = y; i <= z; i++) {
	  j = slope * (i - y) + x;
	  GateDataStructToggleOn(gate_info,gatetype,i,j);
	  // BigDataStructSetVal(gate_info,i,j,BigDataStructGetVal(gate_info,i,j) | (1 << gatetype));
	}
	if (abs(twodmarkers[0].y - firstpoint.y) != 0) {
	  slope = (float) (twodmarkers[0].x - firstpoint.x) / 
	    (float) (twodmarkers[0].y - firstpoint.y);
	} else {
	  slope = 1000000;
	}
	x = Min(twodmarkers[0].y,firstpoint.y);
	z = Max(twodmarkers[0].y,firstpoint.y);
	if (twodmarkers[0].y == x) y = twodmarkers[0].x;
	else y = firstpoint.x;
	for ( i = x; i <= z; i++) {
	  j = slope * (i - x) + y;
	  GateDataStructToggleOn(gate_info,gatetype,j,i);
	  //	  BigDataStructSetVal(gate_info,j,i,BigDataStructGetVal(gate_info,j,i) | (1 << gatetype));
	}
      }
      if ( drawing_gates) {
	TwodCloseGate();
...
void TwodInitializeGate()
{
  ...
  if (gate_info == NULL) {
    gate_info = GateDataStructNew(big_data_info[pgam]->axes[0],big_data_info[pgam]->axes[1],pgammax);
  } else GetMessageDialog("No 2D data in memory.");
}
...
void ReadBig (char *sFilename)
{
  ...
  //--ddc 8jun06 ... AND why would you make it one bigger than pgammax?!  this is the only reason this bug has persisted
  ...
  //					      sizeof(struct bigdata *) * (pgammax + 1));
	...
	*/
	if (test == 0) {
      ...
      fclose(infile);
    } else GetMessageDialog("Error opening file.\n"); // end open file condition
  } else {
  ...
  default:
	...
	for (i = 0; i < big_data_info[l]->axes[0]; i++) {
	  for (j = 0; j < big_data_info[l]->axes[1];j++) {
	    outbuffer[outbuffercounter] = 
	      BigDataStructGetVal(big_data_info[l],j,i);
	    outbuffercounter++;
...
void ReadGates (char *sFilename)
{
  ...
  if (strstr(sFilename,".gde") != NULL) {
	...
	for (k = 0; k < i; k++) {
	  ...
	  l = 0;
	  while (l < j) {
	    test = fscanf(infile,"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
...
void WriteGates (char *sFilename)
{
  ...
  if (gate_info != NULL) {
	...
	WriteTwodText("Successfully wrote out twoddata.\n");
      } else GetMessageDialog("Error Writing file.\n");
    } else {
	...
	fwrite(tempints,sizeof(int),l,outfile);
	fclose(outfile);
      } else GetMessageDialog("Error Writing Gate information to file.\n");
    }
  } else GetMessageDialog("No 2D data in memory.\n");
}
...
void TwodDisplayCurrentRange()
{
  ...
  if (!GTK_IS_WIDGET(Window2D)) DisplayBig();
  else gtk_widget_show(Window2D);
  ...
  if (!GTK_IS_WIDGET(twoddisplayed[0])) {
    ...
    for (i = 0; i < 16; i++) {
      ...
      gtk_widget_set_events(twoddisplayed[i],
      ...
      gtk_table_attach(GTK_TABLE(temptable),twoddisplayed[i],
		       (i % 4),(i % 4) + 1,
		       (i / 4),(i / 4) + 1,
  ...
  for (i = 0; i < numtwoddisplayed; i++) {
    if (big_data_info[twodspectradisplayed[i]] != NULL) {
      ...
      gtk_twodplot_set_x_calibration(GTK_TWODPLOT(twoddisplayed[i]),
				     twodglobalxcalibs[0],
				     twodglobalxcalibs[1],
				     twodglobalxcalibs[2]);
      gtk_twodplot_set_y_calibration(GTK_TWODPLOT(twoddisplayed[i]),
				     twodglobalycalibs[0],
				     twodglobalycalibs[1],
				     twodglobalycalibs[2]);
	...
	gtk_twodplot_set_frac(GTK_TWODPLOT(twoddisplayed[i]),
			      xfraction,yfraction);
      }
	...
	xcurrentrange[1] = big_data_info[twodspectradisplayed[i]]->axes[0] - 1;
      if (ycurrentrange[0] < 0) ycurrentrange[0] = 0;
      if (xcurrentrange[0] < 0) xcurrentrange[0] = 0;
	...
	for (k = xcurrentrange[0]; k <= xcurrentrange[1]; k++) {
	  gtk_twodplot_set_value(GTK_TWODPLOT(twoddisplayed[i]),
				 (k - xcurrentrange[0]),
				 (j - ycurrentrange[0]),
      ...
      gtk_twodplot_set_x_range(GTK_TWODPLOT(twoddisplayed[i]),
			       xcurrentrange[0],xcurrentrange[1]);
      gtk_twodplot_set_y_range(GTK_TWODPLOT(twoddisplayed[i]),
			       ycurrentrange[0],ycurrentrange[1]);
      gtk_twodplot_set_title(GTK_TWODPLOT(twoddisplayed[i]),
			     twodtitles[twodspectradisplayed[i]]);
...
void TwodSetMarker (GtkWidget *widget, GdkEventButton *event)
{
  ...
  if (GTK_IS_WIDGET(twodplot)) {
    ...
    if ((height != 0) && (twodplot->allocation.height !=0)) {
      yresult = ((twodplot->allocation.height - 30) - event->y)/(twodplot->allocation.height - 30) * height
	+ ycurrentrange[0];
    }
...
void TwodDrawMarkers()
{
  ...
  if ((width != 0) && (height != 0)) {
    xresult = ((float)twodmarkers[0].x - (float)xcurrentrange[0]+(float) 0.5) / 
      (float) width * (float)( twodplot->allocation.width - 30) + 30;
    yresult = (float) (twodplot->allocation.height - 30) - 
      ((float)twodmarkers[0].y - (float)ycurrentrange[0] + (float) 0.5) / 
      (float) height * (float) (twodplot->allocation.height - 30);
    if (!(drawing_gates)){
      points[0].x = points[1].x = xresult;
      points[0].y = 0;
      points[1].y = twodplot->allocation.height - 30;
      gdk_draw_lines(twodplot->window,
		     twodplot->style->white_gc,
		     points,2);
      ...
      xresult1 = ((float)twodmarkers[1].x - (float)xcurrentrange[0]+(float) 0.5) / 
	(float) width * (float) (twodplot->allocation.width - 30) + 30;
  ...
  } else GetMessageDialog("Error displaying markers.");
...
/* PgamMatrix2Big
 *
 * Does an internal conversion of the pgammatrix data to the internal big structure for display from "above"
 */
void PgamMatrix2Big()
{
  int i,j,k,l;
  ...
  if(pgammatrixdata.data==NULL) return;
  ...
  case 0:
	...
	for (k = 0; k < compression_factor; k++) {
	  for (l = 0; l < compression_factor; l++) {
	    tempsum += *(pgammatrixdata.data + pgammatrixdata.size * (i * compression_factor + k) + (j * compression_factor + l));
	  }
	}
  ...
  case 3:
	...
	for (k = 0; k < compression_factor; k++) {
	  for (l = 0; l < compression_factor; l++) {
	    ...
	    if (ix == iy) tempsum += *(pgammatrixdata.data + ind);
	  }
	}
	BigDataStructSetVal(big_data_info[pgammax - 1],i,j,tempsum);
      }
...
void DisplayBig()
{
  ...
  if (GTK_IS_WIDGET(Window2D)) return; //--ddc aug11 cleanup.
  ...
  g_signal_connect(GTK_OBJECT(Window2D),"key_press_event",
		       (GCallback) TwodGateSet,NULL);

  //--ddc aug11    gtk_signal_connect(GTK_OBJECT(Window2D),"delete_event",
  g_signal_connect(GTK_OBJECT(Window2D),"delete_event",
		       G_CALLBACK(CloseTwodPlotWindow),NULL);
  ...
  */
  ...
  gtk_box_pack_start(GTK_BOX(twodvbox),
		     gtk_ui_manager_get_widget(menu_manager,"/MainMenu"),
		     FALSE,FALSE,0);
