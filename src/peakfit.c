 ...
 */
//--ddc jan13, add corrections to formula for gaussian area by Pei-Luan Tai
...
void FindPeaks(float minwidth, float maxwidth) {
  ...
  for (Channel = 0; Channel < histsize[spectra]; Channel++) {
    ...
    if ((Left_channel >= 0) && (Right_channel <= histsize[spectra]) && (Width0 > 0)) {
      ...
      //	    } else {
      //	      peak_locs = (int *) realloc(peak_locs,
      //					  sizeof(int) * num_peaks);
      //	    }
...
void AutoCal(int minwidth, int maxwidth) {
  ...
  //	b = b + delb;
  //	//	printf("b = %f.\n",b);
  //	ReturnCloseHits(d,b,c,clist,elist,50);
  //	if (clist->num > 4) {
  //	  PolFit(3,clist->list,elist->list,dummy,clist->num,0,a);
  //	  oldnum = clist->num - 1;
  //	  while (clist->num > oldnum) {
...
void ClosePeakFitWindow(GtkWidget *widget, gpointer data) {
  ...
  gtk_widget_destroy(peakfitentry1);
  peakfitentry1 = NULL;
  gtk_widget_destroy(peakfitentry2);
  peakfitentry2 = NULL;
  gtk_widget_destroy(peakfitentry3);
  peakfitentry3 = NULL;
  for (i = 0; i < 3; i++) {
    gtk_widget_destroy(peakfitentrya[i]);
    gtk_widget_destroy(peakfitentryb[i]);
    gtk_widget_destroy(peakfitentryc[i]);
    peakfitentrya[i] = NULL;
    peakfitentryb[i] = NULL;
    peakfitentryc[i] = NULL;
  }
