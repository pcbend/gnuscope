...
void CreateTitleWindow()
{
  ...
  if (!GTK_IS_WIDGET(manipulatetitlewindow)) {
    ...
    g_signal_connect(GTK_OBJECT(manipulatetitlewindow),"delete_event",
		       G_CALLBACK(HideWidget),NULL);
    ...
    g_signal_connect_swapped(GTK_OBJECT(manipulatetitleentry2),"activate",
			      G_CALLBACK(HideWidget),(GtkObject *)manipulatetitlewindow);
    /* --- butons to go --- */
                     ...
                     GtkWidget *entry)
{
  ...
  if (test >= 3) {  // make sure we got a good input
	...
	newsize = 0;
	for ( i = 0; i < test - 1; i++) {
	  if (histsize[i] > newsize) newsize = histsize[i];
	}
                     ...
                     GtkWidget *entry)
{
  ...
  if (test == 1) {
	...
	temphist[i] = tempsum;
      ...
      for (i = 0; i < histsize[a]; i++) *(histloc[a] + i) = temphist[i];
      currentrange[0] = 0;
...
void GainshiftSpectraEntry (GtkWidget *widget,GtkWidget *entry)
{
  ...
  test = sscanf(gtk_entry_get_text(GTK_ENTRY(entry)),"%d %f %f %f %f %d",
		&a,&oldcal[0],&oldcal[1],&oldcal[2],&newcal,&newchannels);
  printf("Test %d\n",test);
