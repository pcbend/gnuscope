...
void SpawnPrintPrompt(GtkWidget *text) {
  ...
  if ((printcap = fopen("/etc/printcap", "r")) != NULL) {
    while (fgets(dummystr3, 80, printcap) != NULL) {
      if ((strncmp(dummystr3, " ", 1) != 0) || (strncmp(dummystr3, "#", 1) != 0)) {
...
void CreateMainWindow() {
  ...
  g_signal_connect(GTK_OBJECT(mainwindow), "key_press_event", G_CALLBACK(KeyPress), NULL);
  ...
  g_signal_connect(GTK_OBJECT(mainwindow), "delete_event", G_CALLBACK(CloseAppWindow), NULL);
  //--ddc aug11 gtk_signal_connect (GTK_OBJECT (mainwindow), "destroy",
  g_signal_connect(GTK_OBJECT(mainwindow), "destroy", G_CALLBACK(gtk_widget_destroyed),
                   &mainwindow);
  ...
  gtk_table_attach(GTK_TABLE(table), channelinfo, 0, 1, 0, 1, GTK_FILL, GTK_SHRINK, 0, 0);

  /* --- insert the logo in the upper right-hand part of the window --- */

  button = gtk_button_new();

  localimage = gtk_image_new_from_pixmap(
      gdk_pixmap_colormap_create_from_xpm_d(NULL, gdk_rgb_get_colormap(), NULL, NULL, logo_xpm),
      NULL);

  gtk_container_add(GTK_CONTAINER(button), localimage);
  //--ddc aug11  gtk_signal_connect (GTK_OBJECT (button), "clicked",
  g_signal_connect(GTK_OBJECT(button), "clicked", G_CALLBACK(gtk_widget_destroy), NULL);
  gtk_widget_show(localimage);

  gtk_table_attach(GTK_TABLE(table), button, 2, 3, 0, 3, GTK_SHRINK | GTK_FILL,
  ...
  //--ddc gtk2
  {
    ...
    gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(localwindow), maintext);
    pango_layout_get_pixel_size(gtk_label_get_layout(channelinfo), &x, &y);
    gtk_widget_set_size_request(localwindow, -1, 10 * y);
  }
  //--ddc debug
  g_signal_connect(GTK_OBJECT(localwindow), "key_press_event", G_CALLBACK(TextViewKeyCallback),
                   NULL);
  ...
  for (i = 0; i < 16; i++) {
    ...
    gtk_widget_set_events(graphsdisplayed[i],
                          GDK_BUTTON_PRESS_MASK | GDK_POINTER_MOTION_MASK | GDK_KEY_PRESS_MASK);
    //--ddc jun11 gtk2 upgrading, (seperate keypress from accel group?)

    //--ddc aug11 gtk_signal_connect (GTK_OBJECT (graphsdisplayed[i]), "button_press_event",
    //(GtkSignalFunc) SetMarker, NULL);
    g_signal_connect(GTK_OBJECT(graphsdisplayed[i]), "button_press_event", G_CALLBACK(SetMarker),
                     NULL);
    ...
    //    cols = (i - (i %4));
    cols = (i - (i % 4)) / 4;
    gtk_table_attach(GTK_TABLE(displaytable), graphsdisplayed[i], cols, cols + 1, (i % 4),
  ...
  //--ddc apr11 dbg (and general advice for adding container)
  //gtk_container_add(GTK_CONTAINER(box1),displaytable);
  ...
  gtk_window_set_geometry_hints((GtkWidget *)mainwindow, (GtkWidget *)mainwindow, &hints,
  ...
  WriteMainText(
      "\nJanuary 1, 2003\n  Major changes:  PGAM Sort now supports .ev2 data file types.  To use "
      "this add a line containing ev2 or EV2 to then end of your master setup file.\nDecember 30, "
      "2002\n  Major Changes:  1.  Added support for fairly primative autodetection of file types. "
      " It works by looking at the file extention.  If you have not been good about your file "
      "extentions this could cause you problems.  (eg if you have been using .nsm instead of "
      ".NSM).  If this is the case you should use the \"mv\" command in linux to correct, or use "
      "the read file option in the DEGRADED menu.  The autodection of file types also allows the "
      "READ function to be used for .msf, .sf, .ede, .big, .twd, .sqr, and .cmd files.\n2:  "
      "Command file and command line support has been added.  Unfortunately there is no guide to "
      "writing command files yet, and command line options are basically just for reading a file "
      "at start up(which would allow you to do just about everything in conjunction with a command "
      "file).\n3:  Gnuscope can now display more than 1 Density plot.  Unfortunately, screen "
      "resolution has an effect on the maxium which can be easily displayed.  For most computers 8 "
      "is the limit, but the software supports up to 16.  \n4:  Bugs with displaying the gaussians "
      "have been fixed.  \nNovember 22, 2002\n  Major Changes:  1.  Added Spec-U-Lator (press "
      "F12), a graphical interface for the add, subtract, normalize, move and re-title options of "
      "the Manipulate menu.  Try it, You'll like it.\n2:  Added a Veto and Required options to the "
      "PGAM sort window (in the gate section).  These take files which are lists of "
      "<ADC#>,<Min>,<max><CRT>.\nSeptember 24, 2002\n  Major Changes:  Added a \"pairs\" sort "
      "option for pgamsort function.  To use this function, select the appropriate option and "
      "enter the name of a file with pairs information in the entry widget to the right.  The "
      "format of the file is listing the pairs in the form (a,b) where a and b are ADC "
      "numbers.\nSeptember 18, 2002\n  Major Changes:  Histogram display has been modified to "
      "allow for display of negative numbers.  Analysis functions have been appropriately "
      "modified, and an oversubtract when projecting from a pgammatrix will result in negative "
      "counts (as will the MANIPULATE-SUBTRACT command).\nSeptember 17, 2002\n  Major Changes:  "
      "Fixed a bug in the pgamsort routines which was resulting in a compression dependant shift "
      "energy results for sum and double gauss fit.  Also fixed the energy return for gauss fit.\n "
      " Minor Changes:  Clover identification during sorting was changed so that a crystal can now "
      "belong to more than 1 clover.\nSeptember 9, 2002\n  Major Changes:  Re-fresh method for 2D "
      "plots has been changed so the user can see something happening (should be faster too).  "
      "AUTOCAL and PEAKFIT functions have been added under the analysis menu.  To use the AUTOCAL "
      "function you must first read in a file containing information using the READ CALIB INFO "
      "option under the Analysis menu.  Peak fit also requires a FITNESS THRESHOLD which is "
      "related to the signal to noise ration for the peaks.  A recomended starting value is 30.  "
      "The interface should improve in further updates.  \nAugust 19, 2002\n Major Changes:\n the "
      ".ede format has been changed to support titles, the rest of the program will be changed "
      "soon to accomodate titles as well.  If you need a conversion utility for your old .ede "
      "files please let me know by sending e-mail to pavan@nucmar.physics.fsu.edu.  Also, "
      "post-script printing support has been added for both the 1D and 2D histograms.\n\nAugust 9, "
      "2002\n Major Changes:\n   Contours have been added to E-dE plots.  The three plot modes, "
      "listed under Display in the EdE menu, are Contours only, Densities only, and Contours and "
      "Densities.\n   The creation and display of banana gates has also been improved, and will "
      "continue to be.\n\nAugust 6,2002\nMajor Changes:\n   Multiple spectra can now be displayed "
      "in 1 histogram.  Use <Ctrl>+D to display more than 1 histogram in the last spectrum "
      "clicked.\n   Support for reading and writing multiple 2D plots has been added.  Prompting "
      "should be straight forward. \n   Added support for big files in binary format, set as the "
      "default.  The ASCII format is used if the file read in or written out uses the .big file "
      "extention.  The suggested file extention for the binary format is .ede\n\n July "
      "19,2002\nMajor Changes: fixed a bug in E-dE gates\nMinor Change: Improved memory managment "
      "for \"big\" files.\n Previous changes: July,17,2002\n  Major Changes: Fixed a bug in E-dE "
      "2D plot sorting.  Better separation of the contors is now possible.\n");

  WriteMainText(
      "\nJanuary 27, 2003:\n  Major Changes:  Fixed problems with gamma and particle energy gates "
      "inside of a PGAM sort process.  If there are problems please let me know.\n  Minor Changes: "
      " Due to some optimizations sorting (especially energy histogram) should go faster.");
  WriteMainText("\nFebuary 4, 2003:\n(2)\n  Minor Changes: Fixed problem with Gainshift "
                "function.\n(1)\n  Minor Changes:  Fixed bugs in overlaying Histograms and some "
                "bugs in the 2D display which could cause the program to crash.");
  WriteMainText("\nFebuary 12, 2003:\n  Major Changes:  support for wildcard characters in the "
                "read file from command line option.  To use try \"gnuscope -f *.NSM\".\n  Minor "
                "Changes:  Fixed two bugs with double gauss fit.");
  WriteMainText("\nFebuary 17, 2003:\n  Major Changes:  Fixed problems with the construction of "
                "the Spec-U-Lator window.");
  WriteMainText("Febuary 18, 2003:\n  Major Changes:  Fixed an error in the manual set marker "
                "window.  Fixed display problems for gauss, double gauss, exponential fit, and "
                "peakfit displays for semilog and semiroot display modes.");
  WriteMainText("Febuary 25, 2003\n  Major Changes:  Added addback to multi-segment particle "
                "telescopes.  This resulted in a major overhall of how particle telescopes were "
                "sorted.  This is important to you because there may be problems.  If you notice "
                "any please let me know.\n  There was a bug in the 1D Histogram Energy sorting for "
                "multiple telescopes.  This has been fixed.\n");
  WriteMainText(
      "March 4, 2003\n  Major Changes:  Added support for non-particle detector 2D plots.  These "
      "feature can be used by putting an * in the like defining the particle telescope in the "
      "setup file.  2D plots designated in such a way will generate 2D histograms, and can be used "
      "with 2D gates, but will not be included in any of the \"sum\" outputs (most matrices).  The "
      "ADCs used will also not be excluded by any of the \"sum\" outputs.\n  Minor Change:  Added "
      "support for up to 64 2D matricies during sorting.\n");
  WriteMainText(
      "April 11, 2003\n  Major Changes:  For 2D plots.  Added support for both negative numbers, "
      "and interpolation (quadratic).  Quadratic interpolation is turned on by default.  It makes "
      "the plots look better.  However, interpolation is slower than no interpolation, especially "
      "with contour plots.  Therefore, if you can tolerate the difference in quality you might "
      "want to consider no interpolation.\n  Minor Changes:  Changed the way the graphics for the "
      "density plot is drawn.  This should now be faster over the network.\n");
  WriteMainText(
      "May 25, 2003\n  Major Changes:  Well, there are really quite a few.  These are the ones "
      "that I remember.  First, there have been quite a few bugs worked out of sorting from the "
      "ev2 data structures.  There are still occasional screw ups, but Gnuscope now handles these "
      "by dropping the rest of the 8192 byte buffer and starting fresh with the next one.  It does "
      "tell you when it does this, and which byte it was on, so you can have some idea how much "
      "data was lost.  You should be loosing well less than 1% of your data.  Second, the setup "
      "files now have the option of including a maximum reasonable channel in the setup file.  The "
      "purpose of this is to allow the user to throw out overflow events.  This option would "
      "change the second line of the ADC definition of the setup file from \"ADC,CHANNELS,ANGLE\" "
      "to \"ADC,CHANNELS(MAX_CHANNEL),ANGLE\".  Third, the color contrast on the two dimensional "
      "plots has been changed to allow you see low-count channels more easily.  Fourth, Gnuscope "
      "now supports configuration files.  These should be named \".gnuscopeconfig\" and can be "
      "placed in either the directory from which Gnuscope is being read, or in your home "
      "directory.  It will look to the current directory first.  Refer to \"An Almost Certainly "
      "Incomplete Guide to Gnuscope\" for help, or run Gnuscope with the --help flag from the "
      "command line.  Fifth, the Gnuscope Guide, \"An Almost Certainly Incomplete Guide to "
      "Gnuscope\" is avalible either from John Pavan (Me) or online at "
      "\"http://nucalf.physics.fsu.edu/~pavan\".  Supposedly, it is in PDF format, but acroread "
      "can read it, and xpdf cannot.  Finally,  Hmmm...I've fogotten whatelse I've done.  Oh.  "
      "There is tentative (very early) Radware matrix support.  Gnuscope seems to be able to write "
      "out 4Kx4K radware square matricies (from PGAM square matricies) and can read in radware "
      "matricies from the command line.  Eventually, this support will be better.\n");
  WriteMainText(
      "June 8th, 2003\nMajor Changes:  To make it easier on everyone, compress2 has been "
      "incorporated into Gnuscope.  There are two ways to use this feature.  One way is to use it "
      "under the \"File\" pull-down menu.  The other is to use the command line option, \"gnuscope "
      "--compress file.mul --stop\".  Second, there have been some bugs fixed with printing and "
      "with how Gnuscope finds printers.  Third, the 2D display mode now has color scale choices.  "
      "Currently the default is linear, but Log or Root are better for E-DeltaE plots.\n");
  WriteMainText("June 11th, 2003\nMinor Change:  Fixed a bug in marker based subtract gates.\n");
  WriteMainText(
      "June 23rd, 2003\nMajor Changes:  Radware matricies can now be written using the standard "
      "\"Write Histogram\" function.  Several updates to the gtktwodplot.c and gtktwodplot.h which "
      "are hopefully not visible to the end user.Think that is about it.\n");
  WriteMainText(
      "June 25th, 2003\nMajor Change:  Added \"--cnf2txt\" option to the command line.  In "
      "conjunction with the \"--stop\" command line option it can be used to make a batch "
      "conversion of .cnf files to generic ASCII files with the extention of .txt.\n");
  WriteMainText("July 28th, 2003\nMinor Fix:  Gnuscope should no longer be capable of producing a "
                "huge number of error message windows.\n");
  WriteMainText("July 29th, 2003\nMajor Change:  Fixed bug with triples mode for gs92 sorting.\n");
  WriteMainText("November 4th, 2003\nBug Fix:  Fixed compression bug in pgamsort sorting.  Prior "
                "sorts should have calibration \"-(n-1) n 0\" where n is the compression factor.  "
                "New sorts should be able to use \"0 n 0\".\n");
  WriteMainText("Mar, 2005  Bug Fix: A few: misc. pointer errors, double counting in square and "
                "triangle sorting, 'telescopes_hit' initialization.  \n");
  WriteMainText("May, 2005  Bug Fix: Accumulation of roundoff for projected spectra reduced \n");
  WriteMainText(
      "May, 2005  Bug Fix: Detector map underflow references for unused detectors fixed \n");
  WriteMainText("May, 2005  Bug Fix: Addback index error fixed \n");
  WriteMainText("June, 2005  Bug Fix: Detector_info map index underflow when virtual adc numbers "
                "match unused physical adc numbers fixed \n");
  WriteMainText("June, 2005  Bug Fix: 'one off' index error in allocating memory for detectors in "
                "list for square sort AND in singles sort detector maps \n");
  WriteMainText(
      "June, 2005  Bug Fix: gtk indexes out of range in drawing histograms in gtk_graph_draw \n");
  WriteMainText("June, 2005  Bug Fix: Infinite loops in GaussFit and PgamSort broken (input buffer "
                "processing). \n");
  WriteMainText("January, 2006 Bug Fix: Error in writing 'big' files.\n");
  WriteMainText("January, 2006 Bug Fix: Multiple color bugs in twod plots, PLUS implemented "
                "contrast feature.\n");
  WriteMainText("January, 2006 Bug Fix: Bug fix in interpolation routine leading to core dump.\n");
  WriteMainText("January, 2006 Bug Fix: Fixed crash for clearing undefined gates, drawing "
                "undefined spectra.\n");
  WriteMainText("January, 2006 Implementation Change: Gate points are only included AFTER "
                "selecting gating.\n");
  WriteMainText(
      "January, 2006 Implementation Change: Default twod plot mode changed to density only.\n");
  ...
  WriteMainText(
      "May, 2006 Bug Fix: Index test failure in twod histogram sum and difference functions.\n");
  WriteMainText("May, 2006 Bug Fix: Nonnegative values only in difference.\n");
  WriteMainText(
      "May, 2006 Bug Fix: Attempt to reuse gtk objects destroyed by user (by kp), and others...\n");
  ...
  WriteMainText("June, 2006 Bug Fix: Serious memory allocation issue for twod histograms created "
                "by add/sub function.\n");
  WriteMainText("June, 2006 Bug Fix: Array boundary issue for gainshift function.\n");
  WriteMainText("June, 2006 Bug Fix: Multiple uninitialized variables for gate tests for 'Energy "
                "Output' in pgamsort.\n");
  WriteMainText("June, 2006 Logic error: gated gammarays not properly accounted for when placing "
                "gammas in the matrix.\n");
  WriteMainText("August, 2006 Logic error: Fixed failing TAC test when sorting particles.\n");
  WriteMainText("May, 2007 Bug Fixes: Potentially short reads for evt files, and undefined gates "
                "for raw singles sorting.\n");
  WriteMainText("July, 2007 Bug Fixes: No background subtraction from triangle projection, and "
                "error for finding last histogram id.\n");
  WriteMainText(
      "June, 2008 Bug Fixes: Error applying quadratic term for spectrum calibrations, and cleanup "
      "of function prototypes to fix error using commandfiles to enter calibrations\n");
  WriteMainText(
      "April, 2009 Bug Fixes: Dimensionality of radware MAT file fixed (spe files changed to match "
      "and made _binary_).  Redundant windows for twod histograms fixed.\n");
  WriteMainText("April, 2009 Feature: Matrix _subtraction_ added.\n");
  WriteMainText(
      "June, 2009 Bug Fix: 2006 Fix for gainshift out-of-bounds, fixed AND roundoff errors.\n");
  WriteMainText("June, 2012 Fix for using 32k channels, and bugs in binary SPE file reads.\n");
  WriteMainText("July, 2012 Fix for overflows in implicit type conversion for NSM files with 32k "
                "channels.\n");
  ...
  WriteMainText("November, 2014 Fixes from 'pair' testing with V.Tripathi, including 'rare' split "
                "buffer error.\n");
  WriteMainText("Feb, 2015 S.Tabor's grouped doppler correction, plus resizing window, and errors "
                "on menu callbacks.\n");
  ...
  WriteMainText("Jan, 2017 Fix typo in memory allocation pgamsort, and trap histogramming rawsort "
                "ADCs with zero values\n");
