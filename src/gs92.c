...
void ReadGS (char *sFilename)
{
  ...
  if (strstr(sFilename,dummystr) != 0) twoddata.filetype = 1;
  sprintf(dummystr,".sqr");
  if (strstr(sFilename,dummystr) != 0) twoddata.filetype = 2;
...
void ReadSqr(FILE *infile)
{
  ...
  if (twoddata.headbuffer[1] == twoddata.headbuffer[0]) {
    ...
    twoddata.data = (short int *) malloc (twoddata.databuffer[0]);
      ...
      twoddata.data = (short int *) realloc(twoddata.data,
					    twoddata.databuffer[0]);
    }
    fread(twoddata.data,2,(twoddata.size * twoddata.size),infile);
    fread(&twoddata.databuffer[1],4,1,infile);
    if (twoddata.databuffer[0] != twoddata.databuffer[1]) printf("Error reading data.\n");
  } else {
...
void WriteGS (char *sFilename)
{
  ...
  case 1:
    sprintf(dummystr,".twd");
    if (strstr(sFilename,dummystr) == NULL) strcat (sFilename,dummystr);
    if ((outfile = fopen(sFilename,"w")) != NULL) {
      WriteTwd(outfile);
    } else printf("Couldn't open output file.\n");
    break;
  case 2:
    sprintf(dummystr,".sqr");
    if (strstr(sFilename,dummystr) == NULL) strcat (sFilename,dummystr);
...
void DopplerCorrectionEntry(GtkWidget *widget,GtkWidget *entry)
{
  ...
  if ((sorttype == 2) || (sorttype ==3)) GetDialog(23);
}
...
void TwdorSqrEntry (GtkWidget *widget, GtkWidget *entry) 
{
  ...
  if (test == 1) {
    if (strstr(temp,"twd") != 0) twoddata.filetype = 1;
    if (strstr(temp,"sqr") != 0) twoddata.filetype = 2;

    if ((twoddata.filetype == 1) || (twoddata.filetype == 2)) {
      GetFilename("Detector Gate",0,ReadDetectorGte);
      if (sorttype == 3) GetFilename("Gamma Gate",0,ReadGammaGte);
    }
  }
}
...
void Setup()
{
  ...
  if ((1 <= sorttype)&&(4 >= sorttype )){
    ...
    if ((pchannels[0] > 0) && (pchannels[0] <= 20) && 
	(pchannels[1] > 0) && (pchannels[1] <= 20)) {
	...
	if (twoddata.filetype == 1) sprintf(dummystr,".twd");
	if (twoddata.filetype == 2) sprintf(dummystr,".sqr");
	if ((twoddata.filetype == 1) || (twoddata.filetype == 2)) {
	...
	scanf("%s",dummystr);
	sprintf(type,"twd");
	if (strstr(dummystr,type) != NULL) twoddata.filetype = 1;
	sprintf(type,"sqr");
	if (strstr(dummystr,type) != NULL) twoddata.filetype = 2;
	while (!((twoddata.filetype == 1) || (twoddata.filetype == 2))) {
	  printf("What would you like to change the file type to?\n");
	  scanf("%s",dummystr);
	  if (strstr(dummystr,".twd") != NULL) twoddata.filetype = 1;
	  if (strstr(dummystr,".sqr") != NULL) twoddata.filetype = 2;
	}
...
void MakeGateArrays()
{
  ...
  while(gammagates[i].lower != -1) {
	...
	if (gammagates[i].addorsubtract) addarray[j] = 1;
	else if (gammagates[i].addorsubtract == -1) subtractarray[j] = 1;
      }
    }
    i++;
  }
...
void TrackStats ()
{
  ...
  /* --- gamma 4 fold --- */
  for (i =5; i < 10; i++) {
    kstat[5][2] = kstat[5][2] + (float) (kstat[i][1] * 
					 (i - 1) * (i - 2) * (i - 3) * (i - 4))/(float)24;
  }
  printf("No gammas: %d  Gamma Singles: %d  Gamma Doubles:  %d\n",
	 kstat[1][2],kstat[2][2],kstat[3][2]);
...
void Sort()
{
  ...
  if (((pchannels[0] >= 0) && (pchannels[0] < 20))) {  // setup check
	...
	case 1:
	  ...
	  twoddata.databuffer[0] = twoddata.databuffer[1] =
	    (float) sizeof(short int) / (float) 2 *
	    (float) twoddata.size * (float) (twoddata.size + 1);
	  if (twoddata.data == NULL) {
	    twoddata.data = (short int *) malloc(twoddata.databuffer[0]);
	  } else {
	    twoddata.data = (short int *) realloc(twoddata.data,
	  ...
	  if ((infile = fopen(filename,"r")) != NULL) { // file test
		...
		for (k = 0; k <4092; k++) { // interpreting loop
		  ...
		  if (buffer[k] < 0) { // event test
		    ...
		    k++;
		    if (k >= 4092) goto jumpend;  // make sure we arn't going to dump
		    jhk = buffer[k];
		    jh = jhk % 1024;
		    ...
		    krecol = buffer[k];
		    k++;
		    if (k >= 4092) goto jumpend;  // make sure we don't dump
		    kval = buffer[k];
		    if (kval < 0) kval = kval + 65536;
		    ithet = kval % 128;
		    kval = kval - ithet;
		    ...
		    evap = buffer[k];
		    kchn = 0;
		    if (kprot < 5) kchn = kalpha*5 + kprot;
		    /* --- get the gamma energies --- */
		    for (l = 0; l < igam; l++) { // gamma energy loop
		      k++;
		      if (k >= 4092) goto recordgammas;
		      kval = buffer[k];
		      if (kval < 0) kval = kval + 65536;
		      egam[l] = kval % 16384;
		  ...
		  recordgammas:
		    ...
		    if ((kchn >= pchannels[0]) && (kchn <= pchannels[1]) && (ibad < badness)) {
		      ...
		      if (ii <= 20) kstat[ii][1] = kstat[ii][1] + 1;
			...
			for ( l = 0; l < igam; l++) {
			  ener = egam[l];
			  if (beta != 0) DopplerCorrect(&ener,idgam[l]);
			  if ((ener > 0) && (ener < 8192)) {
			...
			if (igam >= 3) { // triples condition
				...
				for (n = ig4; n <= igam; n++) { // triples loop 3
				  ...
				  for (o = 0; o < 3; o++) { // testing loop
					...
					if ((ix > 0) && (ix < twoddata.size) && 
					    (iy > 0) && (iy < twoddata.size)) {// write the event
					  ...
					  DopplerCorrect(&iy,idgam[ng[ks]]);
						...
						*(twoddata.data + twdind) + 1;
					    ...
					    case 2:
					      ...
					      if (beta != 0) DopplerCorrect(&iy,idgam[ng[ks]]);
					      *(twoddata.data + ix * twoddata.size + iy) = 
						*(twoddata.data + ix * twoddata.size + iy) + 1;
					      break;
					    default:
			...
			break;
			  ...
			  for (l = 0; l < igam; l++) { // igam loop
			    if ((idgam[l] >= 1) || (idgam[l] <= 110)) {// idgam condition
			      *(histloc[kk] + 799 + idgam[l]) = 
				*(histloc[kk] + 799 + idgam[l]) + 1;
			    } // end idgam condtion
			  } // end igam loop
			  if ((jh > 0) && (jh < 1000)) { // jh condition
			    *(histloc[kk] + 1000 + jh) = 
			      *(histloc[kk] + 1000 + jh) + 1;
			  } // end jh condition
			  if ((jk > 0) && (jk < 1000)) { // jk condition
			    *(histloc[kk] + 2000 + jk) = 
			      *(histloc[kk] + 2000 + jk) + 1;
			  } // end jk condition
			  if ((ithet > 0) && (ithet < 1000)) { // ithet condition
			    *(histloc[kk] + 3000 + ithet) = 
			      *(histloc[kk] + 3000 + ithet) + 1;
			  } // end ithet condition
			  if ((iphi > 1) && (iphi < 1000)) { // iphi condition
			    *(histloc[kk] + 4000 + iphi) = 
			      *(histloc[kk] + 4000 + iphi) + 1;
			  } // end iphi condition
  ...
  //--ddc gcc 3.4.4 doesn't like labels followed by ... nothing (added continue) 
		...
		if (irec > recordmax) stillreading = 0;
...
void Project()
{
  ...
  case 1:
    ...
    printf("Passing the following information to TwdProject: %d %d %d %d %d %d.\n",
	   addlower,addupper,subtractlower,subtractupper,histdest,add);
		      ...
		      int subtractlower, int subtractupper, int desthist, int add)
{
  ...
  for (i = 0; i < twoddata.size; i++) {
    ...
    for (j = addlower; j <= addupper; j++) {
      ...
      if ((temp < 0) && (twoddata.df > 0)) temp = temp + 65536;
      if (ix == iy) temporary = temporary + temp; /* --- intentional double counting on hypotenus --- */
	...
	temp = *(twoddata.data + ind);
	if ((temp < 0) && (twoddata.df > 0)) temp = temp + 65536;
	if (ix == iy) temporary = temporary - temp * asratio;
 ...
 */
void SqrProject(int addlower, int addupper, int subtractlower, int subtractupper,
